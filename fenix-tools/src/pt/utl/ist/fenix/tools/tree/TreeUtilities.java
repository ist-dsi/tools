...
public class TreeUtilities {
	...
	public void recurseTreeCallMethod(Object rootOfTree, Method mGetChildsMethod, Object callMethodTarget,
		...
		try {
			...
			Object[] argsAtLevel = new Object[(callMethodTarget == null ? 0 : 1)
					+ (additionalParametersToTargetMethod == null ? 0 : additionalParametersToTargetMethod.length)];

			if (callMethodTarget != null) argsAtLevel[0] = rootOfTree;

			if (additionalParametersToTargetMethod != null)
				System.arraycopy(additionalParametersToTargetMethod, 0, argsAtLevel,
						(callMethodTarget == null ? 0 : 1), additionalParametersToTargetMethod.length);
			...
			if (oChilds != null) {
				if (oChilds instanceof Iterable) {
					...
					for (; iteratorChilds.hasNext();) {
						Object child = (Object) iteratorChilds.next();
						recurseTreeCallMethod(child, mGetChildsMethod, callMethodTarget, mCallMethodAtLevel,
								additionalParametersToTargetMethod);
					}
				}
			...
			TreeRecurseException e2 = new TreeRecurseException("illegal argument invoking method",
					pathRecurseComponents, e);
			pathRecurseComponents.clear();
			throw e2;
		}
		catch (IllegalAccessException e) {
			TreeRecurseException e2 = new TreeRecurseException("illegal access invoking method", pathRecurseComponents,
					e);
			pathRecurseComponents.clear();
			throw e2;
		}
		catch (InvocationTargetException e) {
			TreeRecurseException e2 = new TreeRecurseException("target object does not have method",
					pathRecurseComponents, e);
			pathRecurseComponents.clear();
			throw e2;
		}
	...
	public void recurseTreeCallMethod(Object rootOfTree, String strGetChildsMethod, Object callMethodTarget,
		...
		try {
			...
			if (callMethodTarget != null) {
				...
				Class[] argTypes = new Class[1 + (additionalParametersTypesToTargetMethod == null ? 0
				...
				// There is no callMethodTarget, so the method should be called on each
				// object in the tree (so instrospect it from the tree Object -
				// we don't expect to pass in the Object itself, as the method is being
				// called on the correct object allready)
				Class[] argTypes = new Class[(additionalParametersTypesToTargetMethod == null ? 0
						: additionalParametersTypesToTargetMethod.length)];
				if (additionalParametersTypesToTargetMethod != null)
	...
	public Collection<? extends Object> recurseToRoot(Object child, String strGetParentMethod)
			throws TreeRecurseException {
		class PathSaver {
