...
public class RemoteStreamHandlerFactory {
	...
	public synchronized void initRemoteStreamsHandler(long remoteStreamsHandlerMaxIdleInterval,
			long remoteStreamsHandlerCleaningInterval) {
		if (remoteStreamHandler == null) {
			...
			remoteStreamHandler =
					new RemoteStreamsHandlerImpl(remoteStreamsHandlerMaxIdleInterval, remoteStreamsHandlerCleaningInterval);
	...
	private synchronized RemoteStreamsHandlerImpl getRemoteStreamsHandler() {
		if (remoteStreamHandler == null) {
			remoteStreamHandler =
					new RemoteStreamsHandlerImpl(DEFAULT_REMOTE_STREAMS_IDLE_TIMEOUT, DEFAULT_REMOTE_STREAMS_IDLE_CHECK_INTERVAL);;
			remoteStreamHandler.start();
		}
