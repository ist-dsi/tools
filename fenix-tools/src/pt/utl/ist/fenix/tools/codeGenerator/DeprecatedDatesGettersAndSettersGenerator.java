    ...
    public void appendMethodsInTheRootDomainObject() throws IOException {
        ...
        for (Iterator<DomainClass> iter = getModel().getClasses(); iter.hasNext();) {
            ...
            if (resultSourceCode.length() > 0) {
                ...
                String domainClassSourceCodeFilePath =
                        outputFolder + "/" + domainClass.getFullName().replace('.', '/') + sourceSuffix;
        ...
        for (Iterator<Slot> iter = domainClass.getSlots(); iter.hasNext();) {
            ...
            if (slot.getType().equals("org.joda.time.YearMonthDay")) {
                ...
                if (originalSlotName.endsWith("YearMonthDay")) {
                    ...
                    methods.format("\t\treturn (ymd == null) ? null : new java.util.Date(ymd.getYear() - 1900, ymd.getMonthOfYear() - 1, ymd.getDayOfMonth());\n");
                    methods.format("\t}\n");
                ...
                if (originalSlotName.endsWith("DateTime")) {
                    ...
                    methods.format("\t\treturn (dt == null) ? null : new java.util.Date(dt.getMillis());\n");
                    methods.format("\t}\n");
                ...
                if (originalSlotName.endsWith("HourMinuteSecond")) {
                    ...
                    methods.format("\n\t@Deprecated\n\tpublic java.util.Date get%s(){\n", dateSlotName);
                    methods.format("\t\tnet.sourceforge.fenixedu.util.HourMinuteSecond hms = get%s();\n", originalSlotName);
                    methods.format("\t\treturn (hms == null) ? null : new java.util.Date(0, 0, 1, hms.getHour(), hms.getMinuteOfHour(), hms.getSecondOfMinute());\n");
                    methods.format("\t}\n");

                    methods.format("\n\t@Deprecated\n\tpublic void set%s(java.util.Date date){\n", dateSlotName);
                    methods.format("\t\tif(date == null) set%s(null);\n", originalSlotName);
                    methods.format("\t\telse set%s(net.sourceforge.fenixedu.util.HourMinuteSecond.fromDateFields(date));\n",
                            originalSlotName);
                    methods.format("\t}\n");
                }
            }
