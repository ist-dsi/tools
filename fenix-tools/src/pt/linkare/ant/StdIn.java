...
public class StdIn {
	...
	public boolean readBooleanOption(String message, String yesOption, String noOption) {
		...
		while (true) {
			try {
				...
				if (readLine != null) {
					if (readLine.equals(yesOption)) return true;
					else if (readLine.equals(noOption)) return false;
					else System.out
							.println("Invalid value. Valid Options are: [" + yesOption + "," + noOption + "]");
				}
			}
			catch (Exception e) {
				// ignored
	...
	public int readMenuOption(String message,String[] optionsMessages) {
		...
		while (true) {
			//by the time we get here... the user may select any option between 1 and currentOptionCount (inclusive)
			
			try {
				String readLine = readStringFromInput(fullMessage.toString());
				if (readLine == null || readLine.length() <=0) {
					System.out.println(readLine == null ? "You must specify an option!" : "Valid options are between 1 and "+currentOptionCount);
					continue;
				}
				int valueSelected=-1;
				try
			...
			try {
				...
				if (readLine != null) {
					if (readLine.equals(yesOption)) return true;
							...
							.println("Invalid value. Valid Options are:[" + noOption + "," + yesOption + "]");
				}
			}
			catch (Exception e) {
				// ignored
	...
	public int readMenuOptionOrQuitOrDefault(String message,String[] optionsMessages,String quitOption,int defaultOption) {
		...
		{
			++currentOptionCount;
			fullMessage.append(CRLF).append(defaultOption==currentOptionCount?"[":" ").append(currentOptionCount).append(" - ").append(currentOptionMessage).append(defaultOption==currentOptionCount?"]":"");
		}
		fullMessage.append(CRLF).append(quitOption).append(" - Quit!");
	...
	public String readMultipleOption(MenuMessage menuMessage) {
		...
		while (true) {
			...
			try {
				String readLine = readStringFromInput(fullMessage.toString());
				if (readLine == null || readLine.length() <=0) {
					System.out.println("You must choose at least one value between 1 and "+currentOptionCount+" or q (quit with no selection)!");
					continue;
				}
				String[] values=readLine.split(",");
	...
	public String readMenuOptionOrQuit(MenuMessage menuMessage) {
		...
		while (true) {
			...
			try {
				String readLine = readStringFromInput(fullMessage.toString());
				if (readLine == null || readLine.length() <=0) {
					System.out.println("You must choose a value between 1 and "+currentOptionCount+" or q (quit with no selection)!");
					continue;
				}
				if(readLine.equalsIgnoreCase("q"))
					return null;
