...
public class MergeTextFilesTask extends Task {
	...
	private void log(List<TextBlock> merge, List<TextBlock> undefinedOnImport) {
		if (getLogFile() != null) {
			...
			pw.println("Logging merge of file " + originalFile.getPath() + " with properties from " + mergeFile.getPath());
	...
	private List<TextBlock> loadFile(File file) throws BuildException {
		try {
			return TextBlockParser.TextBlockParserManager.getBlockParserByFileName(file.getName()).readBlocks(
	 ...
	 */
	private void mergeTextBlock(List<TextBlock> source, List<TextBlock> merge, List<TextBlock> undefinedOnImport)
			throws BuildException {
	 ...
	 */
	private void writeOutFile(List<TextBlock> source, List<TextBlock> merge) throws BuildException {
